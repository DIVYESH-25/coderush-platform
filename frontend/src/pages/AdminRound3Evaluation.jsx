...
const AdminRound3Evaluation = () => {
    ...
    const handleViewSubmission = (team) => {
        ...
        const initialScores = [0, 1].map(i => team.round3QuestionScores?.[i] || 0);
    ...
    const handleSaveScores = async () => {
        try {
            ...
            alert('Round 3 scores saved successfully!');
    ...
    if (selectedTeam) {
        const r3Total = scores.reduce((a, b) => a + b, 0);
        ...
        return (
            <div className="fade-in" style={{ paddingBottom: '50px' }}>
                ...
                <h2 className="neon-text">Evaluating Round 3: {selectedTeam.teamName}</h2>

                {/* Display solutions */}
                {[0, 1].map((i) => (
                    <div key={i} className="glass-panel" style={{ marginTop: '20px' }}>
                        <h3>Problem {i + 1}</h3>
                        <div style={{ height: '300px', margin: '15px 0', border: '1px solid var(--electric-blue)' }}>
                            <Editor
                                height="100%"
                                language={i === 0 ? 'python' : 'java'}
                                theme="vs-dark"
                                value={selectedTeam.round3Solutions?.[i] || 'No submission'}
                                options={{ readOnly: true, minimap: { enabled: false } }}
                            />
                        </div>
                        <div>
                            <label style={{ marginRight: '10px', fontWeight: 'bold' }}>Score (0-50):</label>
                            <input
                                type="number"
                                min="0" max="50"
                                value={scores[i]}
                ...
                <div className="glass-panel" style={{ marginTop: '30px', textAlign: 'center' }}>
                    <h3 style={{ marginBottom: '15px' }}>Total Round 3 Score: <span style={{ color: 'var(--success)' }}>{r3Total} / 100</span></h3>
    ...
    return (
        <div className="fade-in">
            ...
            <h2 className="neon-text" style={{ marginBottom: '30px' }}>Round 3 Evaluation</h2>
            <div className="glass-panel">
                <table style={{ width: '100%', borderCollapse: 'collapse', textAlign: 'left' }}>
                    <thead>
                        <tr style={{ borderBottom: '1px solid var(--electric-blue)' }}>
                            <th style={{ padding: '12px' }}>Team Name</th>
                            <th style={{ padding: '12px' }}>Round 3 Status</th>
                    ...
                    <tbody>
                        {teams.map(team => (
                            <tr key={team.teamId} style={{ borderBottom: '1px solid rgba(255,255,255,0.1)' }}>
                                ...
                                <td style={{ padding: '12px', color: team.round3Completed ? 'var(--success)' : 'var(--text-secondary)' }}>
                                    {team.round3Completed ? 'Submitted' : 'Pending'}
                                </td>
                                <td style={{ padding: '12px', fontWeight: 'bold' }}>{team.round3Score || 0}</td>
                                <td style={{ padding: '12px' }}>
                                    <button
                                        className="btn-neon"
                                        onClick={() => handleViewSubmission(team)}
                                        disabled={!team.round3Completed}
                                        style={{ opacity: team.round3Completed ? 1 : 0.5 }}
                                    >
                                        View Submission
                                    </button>
                                </td>
...
export default AdminRound3Evaluation;
